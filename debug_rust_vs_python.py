def debug_comparison(self_number, self_side, other_number, other_side):
    """Debug the exact Python comparison logic step by step"""
    print(f"\n=== Comparing ('{self_number}', {self_side}) < ('{other_number}', {other_side}) ===")
    ...
    self_is_pure_digit = self_number == self_number_clean
    other_is_pure_digit = other_number == other_number_clean
    
    print(f"self_is_pure_digit: {self_is_pure_digit}")
    print(f"other_is_pure_digit: {other_is_pure_digit}")

    # Case 1: Both are pure digits
    if self_is_pure_digit and other_is_pure_digit:
        print("CASE 1: Both are pure digits")
    ...
    if self_is_pure_digit:
        print("CASE 2: Self is pure digit, other is not")
        if self_number_clean_int == other_number_clean_int:
    ...
    if other_is_pure_digit:
        print("CASE 3: Other is pure digit, self is not")
        if self_number_clean_int == other_number_clean_int:
    ...
    print("CASE 4: Neither is pure digit")
    ...
    # Then check if integer values are the same
    if self_number_clean_int == other_number_clean_int:
...
for self_num, self_side, other_num, other_side in test_cases:
    result = debug_comparison(self_num, self_side, other_num, other_side)
    print(f"FINAL RESULT: ('{self_num}', {self_side}) < ('{other_num}', {other_side}) = {result}")