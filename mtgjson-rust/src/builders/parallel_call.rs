...
pub fn parallel_call(
    ...
    let rt = tokio::runtime::Builder::new_multi_thread()
        .worker_threads(pool_size.min(num_cpus::get()).max(1))
        .enable_time()
        .enable_io()
        .thread_name("mtgjson-parallel")
        .build()
        .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(
            format!("Failed to create async runtime: {}", e)
        ))?;
    
    // Execute async parallel processing
    rt.block_on(async move {
        // Create semaphore for concurrency control with backpressure
        let semaphore = Arc::new(Semaphore::new(pool_size));
        let mut join_set = JoinSet::new();
        
        // Optimized dispatch based on parameter combinations
                ...
                for arg in args.iter() {
                    ...
                    join_set.spawn(async move {
                        ...
                        Python::with_gil(|py| -> PyResult<PyObject> {
                            // Build argument list: [arg] + repeatable_args
                            let mut call_args = vec![arg_obj];
