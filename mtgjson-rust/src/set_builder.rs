use crate::classes::{MtgjsonCardObject, MtgjsonDeckObject, MtgjsonForeignDataObject, MtgjsonGameFormatsObject, MtgjsonLeadershipSkillsObject, MtgjsonLegalitiesObject, MtgjsonMetaObject, MtgjsonRelatedCardsObject, MtgjsonRulingObject, MtgjsonSealedProductObject, MtgjsonSetObject, MtgjsonTranslationsObject};
...
pub async fn parse_rulings(rulings_url: &str) -> Vec<MtgjsonRulingObject> {
            ...
            if let Some(data) = rules_api_json["data"].as_array() {
                for sf_rule in data {
                    if let (Some(date), Some(text)) = (
                        ...
                        let ruling = MtgjsonRulingObject {
                            date: date.to_string(),
                            text: text.to_string(),
                        };
...
pub fn add_uuid(mtgjson_card: &mut MtgjsonCardObject) {
...
pub async fn build_mtgjson_set(set_code: &str) -> Option<MtgjsonSetObject> {
...
pub async fn build_base_mtgjson_cards(
    ...
    let mut mtgjson_cards: Vec<MtgjsonCardObject> = card_results
        .into_iter()
        .flatten()
        .collect();
...
pub fn get_base_set_size(mtgjson_set: &MtgjsonSetObject) -> i32 {
...
pub async fn add_is_starter_option(mtgjson_set: &mut MtgjsonSetObject) {
    if let Some(ref code) = mtgjson_set.code {
...
pub fn handle_special_set_cases(mtgjson_set: &mut MtgjsonSetObject) {
    if let Some(ref code) = mtgjson_set.code {
...
pub fn add_meld_face_parts(mtgjson_set: &mut MtgjsonSetObject) {
    if let Some(ref code) = mtgjson_set.code {
...
pub fn add_secret_lair_names(mtgjson_set: &mut MtgjsonSetObject) {
    if let Some(ref code) = mtgjson_set.code {
